...
import {ClientDialogEditComponent} from './client-dialog-edit/client-dialog-edit.component';
import {
  ConfirmDeleteData,
...
export class ClientComponent {
  ...
  displayedColumns = ['id', 'name', 'telephone', 'actions'] as const;
  ...
  fetch(page: number, size: number, sort?: Sort, filtro?: any) {
    ...
    this.service.listarClientes(filtro ?? {}, page, size, sortData).subscribe({
      next: ({ content, total }) => {
  ...
  trackById = (_: number, item: Client) => item.id!;
  confirmDelete(row: { id: number; name?: string }) {
    const ref = this.dialog.open(ConfirmDeleteDialogComponent, {
      width: '420px',
      data: {
        title: 'Excluir cliente',
        message: `Tem certeza que deseja excluir "${row.name ?? 'este cliente'}" (Código: ${row.id})?`,
        confirmText: 'Excluir',
        cancelText: 'Cancelar',
  ...
  delete(id: number) {
    ...
    this.service.excluir(id).subscribe({
      ...
      error: (err) => {
        ...
        alert(err?.error?.message || err?.message || 'Erro ao excluir cliente');
      },
    });
  }
...
openEditDialog(id?: number) {
  if (!id) return;
  const ref = this.dialog.open(ClientDialogEditComponent, {
    width: '960px',
    maxWidth: '98vw',
    maxHeight: '92vh',
    disableClose: true,
